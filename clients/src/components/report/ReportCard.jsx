 ...
 * @param {(record: object) => void} props.onEdit - Callback khi bấm nút Chỉnh sửa
 */
export default function ReportCard({ record, isPending, onEdit }) {
  const {
    id,
    stt,
    hoTen_ThuocCtyDonVi,
  ...
  return (
    <div className="group relative rounded-xl border border-border bg-card p-4 shadow-sm transition-shadow hover:shadow-md">
      {/* Nút Chỉnh sửa — chỉ hiện trên card pending khi hover/focus */}
      {isPending && onEdit && (
        <button
          type="button"
